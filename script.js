...
function renderFeedbackButtons(hitId, ruleId, inputText) {
  const hash = btoa(inputText.substring(0,100));
...
function scoreHit(text,rule,mc,contra,ctx){
  ...
  // NEW: Heavily penalize the "Supervisor Wrong" phone rule if the user admits to personal apps
  if(rule.id==="phone" && /personal|social media|instagram|tiktok|facebook|snapchat|youtube|scrolling/.test(text)) score-=80;

  // NEW: Massive boost to the "You Violated Policy" rule if personal apps are explicitly mentioned
  if(rule.id==="personal_phone_misuse" && /personal|social media|instagram|tiktok|facebook|snapchat|youtube|scrolling/.test(text)) score+=60;
...
function go(text){
  ...
  setTimeout(function(){
    ...
    // attach feedback listeners after rendering
